...
pipeline {
    ...
    stages {
        ...
        stage("Paso 0: Download Code and checkout"){
            ...
            post{
				success{
					slackSend color: 'good', channel: "${env.channel}", message: "[${GROUP}][${env.JOB_NAME}][Pipeline IC/CD][Rama:${env.BRANCH_NAME}][Stage:${STAGE}][Resultado:Éxito/Success]", teamDomain: 'devopsusach20-lzc3526', tokenCredentialId: 'slack-angelo-channel'
				}
				failure{
					slackSend color: 'danger',channel: "${env.channel}", message: "[${GROUP}][${env.JOB_NAME}][Pipeline IC/CD][Rama:${env.BRANCH_NAME}][Stage:${STAGE}][Resultado:Error/Fail]", teamDomain: 'devopsusach20-lzc3526', tokenCredentialId: 'slack-angelo-channel'
				}
        }
    }
